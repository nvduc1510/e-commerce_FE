            ...
            <div className="group">
                <div className="relative overflow-hidden">
                    <Link to={`/product-details/${item.slug}`}>
                        <img className="w-full transform group-hover:scale-110 duration-300" src={`${imageBasePath}` + item.thumbnailImage} alt="shop"/>
                    </Link>
                    {/* {item.discount === 0 &&
                    ...
                    <div className="absolute z-10 top-[50%] right-3 transform -translate-y-[40%] opacity-0 duration-300 transition-all group-hover:-translate-y-1/2 group-hover:opacity-100 flex flex-col items-end gap-3">
                        ...
                        <Link to={`/product-details/${item.slug}`} className="bg-white dark:bg-title dark:text-white bg-opacity-80 flex items-center justify-center gap-2 px-4 py-[10px] text-base leading-none text-title rounded-[40px] h-14 overflow-hidden new-product-icon quick-view">
                ...
                <div className="md:px-2 lg:px-4 xl:px-6 lg:pt-6 pt-5 flex gap-4 md:gap-5 flex-col">
                    <h4 className="font-medium leading-none dark:text-white text-lg">{formatCurrency(item.price)}</h4>
                    <div>
                        <h5 className="font-normal dark:text-white text-xl leading-[1.5]">
                            <Link to={`/product-details/${item.slug}`} className="text-underline">{item.productName}</Link>
                        </h5>
