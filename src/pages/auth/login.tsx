...
import Aos from "aos";
import { UserLogin } from '@/model/request/utilRequest';
import {apiAuth} from '@/service/commonApi'
...
export default function Login() {
    ...
    const handleLogin = async(e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const response = await apiAuth.handleLogin(userData);
            console.log("success");
            sessionStorage.setItem("access_token", response.data.accessToken);
            sessionStorage.setItem("token_type", response.data.tokenType);
                    ...
                    <div className="mx-auto md:mx-0 max-w-md">
                        ...
                        <form onSubmit={handleLogin}>
                            ...
                            <div data-aos="fade-up" data-aos-delay="600">
                                <button type="submit" className="btn btn-theme-solid mt-[15px]" data-text="Login"><span>Login</span></button>
                            </div>
                        </form>
