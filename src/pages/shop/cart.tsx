...
import { fetchItemFormCart } from "@/store/cartThunks";
import { getUserIdFromToken } from "@/components/utils/auth/tokenUtils";
import { formatCurrency } from '@/components/utils/commonFormat';
...
export default function Cart() {
    ...
    useEffect(()=>{
        if(userId != null) {
            dispatch(fetchItemFormCart(userId))
        }        
        Aos.init()  
    },[userId, dispatch])
    return (
                                            ...
                                            <td className="md:w-[42%]">
                                                <div className="flex items-center gap-3 md:gap-4 lg:gap-6 cart-product my-4">
                                                    ...
                                                    <div className="flex-1">
                                                        <h6 className="leading-none font-medium">{cart?.product?.productName}</h6>
                                                        <h5 className="font-semibold leading-none mt-2"><Link to="#">{cart?.product?.category.categoryName}</Link></h5>
                                                    </div>
                                                </div>
                                            </td>
                                            <td>
                                                {/* <ProductPrice price={cart.product?.price ?? 0}
                                                        discount={cart.product?.discount ?? 0} /> */}
                                                {
                                                    cart.product?.discount && cart.product?.discount > 0 ? (
                                                        <h6 className="text-base md:text-lg leading-none text-title dark:text-white font-semibold">
                                                            {formatCurrency(cart.product.price * (1 - cart.product.discount / 100))}
                                                        </h6>
                                                    ) : (
                                                        <h6 className="text-base md:text-lg leading-none text-title dark:text-white font-semibold">
                                                            {formatCurrency(cart.product?.price ?? 0)}
                                                        </h6>
                                                    )
                                                }   
                                            </td>
